...
//拼接路径至 ffmpeg 指令，并输入终端
    if (is_regular_file(input) || is_directory(input))
        for (const directory_entry& each : recursive_directory_iterator(input)) {       //递归遍历文件/文件夹
            if (is_video(each))                                                         //判断是否为视频
                system(std::format(R"(ffmpeg -i "{}" -map 0 -map_metadata 0 -c copy -c:v libx265 -crf 23 -preset medium -cues_to_front yes "{}.mkv")", each.path().string(), (each.path().parent_path()/each.path().stem()).string()));
        }
    else std::cout << "error";
//跳转，重新读取用户输入
    goto label;
